...
namespace Mercado_libre_frontend.Controllers
{
    public class UsuariosController(UsuariosClientService usuarios, RolesClientService roles) : Controller
    {
        ...
        public async Task<IActionResult> CrearAsync(UsuarioPwd itemToCreate)
        {
            ...
            ModelState.AddModelError("Email", "No ha sido posible realizar la accion. Intentelo nuevamente.");
            await RolesDropDownListAsync();
        ...
        public async Task<IActionResult> EditarAsync(string email)
        {
            ...
            try
            {
                item = await usuarios.GetAsync(email);
                if (item == null)
                {
                    return NotFound();
                }
            ...
            await RolesDropDownListAsync(item?.Rol);
            return View(item);
        }
